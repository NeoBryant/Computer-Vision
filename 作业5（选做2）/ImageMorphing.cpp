...
ImageMorphing::ImageMorphing(const char* srcFilename, const char* dstFilename) {
	src.load(srcFilename);
    dst.load(dstFilename);
	this->frame_cnt = 11; //默认中间11帧
...
// 对每一帧进行morphing
CImgList<float> ImageMorphing::meshBasedMorphing() {
	readPointAndTrianglesPoints(); //读点集
	setSrcTriangleList();
	setDstTriangleList();
	setMidTriangleList();
	...
	for (int k = 0; k < frame_cnt; k++) {
		...
		result.push_back(middle); // 放入中间变换的每一帧
	}
	result.push_back(dst); // 放入最后一帧
	cout << "Morphing successfully!" << endl;
...
//读取数据点
void ImageMorphing::readPointAndTrianglesPoints(){
	// read source points
...
void ImageMorphing::saveImgs(){
	string dir_name;
	for (int i = 0; i < result.size(); i++) {
		dir_name = "result/" + to_string(i+1) + ".bmp";
		result[i].save_bmp(dir_name.c_str()); // Save every frame
	}
	cout << "Save images in /result" << endl;
}

//像素值直接叠加变换函数
CImgList<float> ImageMorphing::crossDisolveMorphing(){
    CImg<float> middle(dst._width, dst._height, 1, 3, 1); //创建三通道rgb图
	
    //中间图像转换
    for (int i = 0; i < frame_cnt+2; ++i){ //中间有11帧
        cimg_forXYC(middle, x, y, c){
            middle(x,y,c) = (unsigned char)((1-1.0*i/(frame_cnt+1))*src(x,y,c) + 1.0*i/(frame_cnt+1)*dst(x,y,c));
        }
        result.push_back(middle); // 放入中间变换的每一帧
    }
	cout << "Morphing successfully!" << endl;
