...
CImg<float> myhough::run(CImg<float> thres, int scale) {
	this->thres = thres;
...
// 映射到霍夫空间
void myhough::houghLinesTransform(CImg<float> &imgin) {
...
// 获取一个窗口内最大权重
int myhough::getMaxValue(CImg<float> &img, int &size, int &y, int &x) {
	int max = 0;
...
void myhough::findPoint(int scale) {
...
vector<pair<int, int> > myhough::getCorner() {
	return this->corner;
}
#endif