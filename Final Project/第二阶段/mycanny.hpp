...
private:
	...
	int scale;
public:
	CImg<float> run(string filename);
	CImg<float> toGrayScale(const CImg<float>& input);//change all pixel into grayscale
...
int mycanny::getScale() {
	return scale;
}

CImg<float> mycanny::run(string filename) {
	//预处理
	CImg<float> image;
	...
	CImg<float> grayImage = toGrayScale(temp); // 转灰度图
...
CImg<float> mycanny::toGrayScale(const CImg<float>& input) {
...
vector<vector<float> > mycanny::createFilter(int row, int column, float sigmaIn) {
	vector<vector<float> > filter;
	for (int i = 0; i < row; i++) {
		vector<float> col;
...
CImg<float> mycanny::useFilter(CImg<float> & img_in, vector<vector<float> >& filterIn) {
...
CImg<float> mycanny::sobel(CImg<float>& gFiltered, CImg<float>& angles) {
	//Sobel X Filter
	float x1[] = {-1.0, 0, 1.0};
	float x2[] = {-2.0, 0, 2.0};
	float x3[] = {-1.0, 0, 1.0};
...
CImg<float> mycanny::nonMaxSupp(CImg<float>& sFiltered, CImg<float> & angles) {
...
CImg<float> mycanny::threshold(CImg<float>& imgin, int low, int high)
{
	if(low > 255) low = 255;
