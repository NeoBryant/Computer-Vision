#ifndef MYIMAGEWARPING_H
#define MYIMAGEWARPING_H
...
CImg<float> myImageWarping::run(string filename, vector<pair<int,int> > corner) {
	...
	int A4_w = 1260, A4_h = 1782;
	if (this->direction == 0) 
		this->result = CImg<float>(A4_w, A4_h, 1, 3, 0);
	else 
		this->result = CImg<float>(A4_h, A4_w, 1, 3, 0);
...
vector<pair<int,int> > myImageWarping::SortCorner(vector<pair<int,int> >& corner) {
...
vector<float> myImageWarping::ComputeMatrix(vector<pair<int,int> > uv) {
	// 初始化投影坐标
...
pair<int, int> myImageWarping::Transform(vector<float> matrix, pair<int,int> point)
{
	int u = point.first;
	int v = point.second;
